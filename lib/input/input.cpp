...
/*
  ...
  Inoltre vi è un controlllo della durata del tempo di acquisiszione con il contatore che viene resettato ad ogni pressione di tasto.
  ...
  while (newData == false) { //blocco di lettura
    ...
    while ( Serial.available()) {// blocco di acquisizione dei caratteri
      ...
      if (rc == endMarker1 || rc == endMarker2) {          // if a CR or LF was received
        ...
        ndx = ndx - 1;      	                     // Set the Array-Pointer back to delete the last characte
        ...
        if (__HideEcho__ == false) {                 // Hide echo if user types in a password...
          Serial.print(rc);
        }
        else {                                       // Normal echo
          Serial.print("*");
